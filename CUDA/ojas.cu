...
__global__ void ojas_rule_par(float *x, float *w, const float learning_rate, const int num, const int len) {
    float *y_arr;
    float y;
    float *x_start;
    y_arr = new float[len];
...
__host__ void run_ojas(float *w, std::vector<float> vec_x, const int num, const int len, const bool par_y = false) {
    ...
    if (par_y) {
        ...
        ojas_rule_par<<<1, 1>>>(d_x, d_w, learning_rate, num, len);
    }
