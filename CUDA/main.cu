...
int main() {
    ...
    const int len = (5*5);                        //lengden på et patch/bilde: Patch:(5*5), Bilde:(28*28)
    ...
    cudaProfilerStart();
    run_ojas(w, x, num_seg, len, true, num_neurons); //Bør settes til true dersom man ønsker å regne ut y på en parallellisert måte
    cudaProfilerStop();
